...
router.put('/:spotId', requireAuth, async (req, res, next) => {
    ...
    if (Number(lat) < -90 || Number(lat) > 90) errors["lat"] = "Latitude must be within -90 and 90";
    if (Number(lng) < -180 || Number(lat) > 180) errors["lng"] = "Longitude must be within -180 and 180";
    if (name.toString().length > 50) errors["name"] = "Name must be less than 50 characters";
    if (!description) errors["description"] = "Description is required";
    if (Number(price) < 0) errors["price"] = "Price per day must be a positive number";
